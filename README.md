...
- 그 순위를 **실시간 · 오늘 · 주간 · 월간 · 연간**으로 나눠 볼 수 있습니다
...
| 📊 **뉴스 통계 보기** | 위 지도 화면 상단의 **[통계]** 버튼 |
| 🔎 **보관 기사 검색** — 제목·연도·공원·분야로 | **<https://parknews.vercel.app/search>** |
| 📥 **뉴스 데이터 내려받기(CSV)** | **<https://parknews.vercel.app/data>** |

> 💡 즐겨찾기에 추가해두고 쓰시면 편합니다.
> 사내망 GitLab에는 같은 소스가 보관되어 있습니다 (아래 *담당자용* 참고).
...
| 화면 위 **[밝은 지도]** | 밝은 지도 ↔ 어두운 지도 전환 · **기본은 밝은 지도**입니다 (선택은 다음 방문에도 유지) |
...
> ⏳ 매일 아침 하루치씩 쌓이고, 그 이전은 **뉴스 아카이브(자료받기와 같은 데이터)로 사후 집계**해 채웠습니다.
> 그래서 월간·연간도 처음부터 온전하게 나옵니다. 사후 집계분은 표본이 달라(네이버 검색 수집)
> 언론사 수가 실제보다 보수적으로 잡힐 수 있고, 빠진 날이 있으면 화면에 그대로 표시됩니다.

### 4. 안전 캘린더 — "이 시기엔 어떤 위험이 많았나"

지도 오른쪽 아래에 뜨는 카드입니다. 1990년부터의 아카이브에서 **재난안전·구조활동
기사 8천여 건**을 위험 유형(산불 · 호우·태풍 · 폭설·한파 · 물놀이 · 실족·추락 ·
벌쏘임·동물피해 등 13종)으로 나눠 월별로 미리 집계해 두고, **지금 달에 보도가
몰렸던 위험**부터 보여줍니다. 수집한 데이터를 쌓아두는 데서 끝내지 않고
"그래서 언제 무엇을 조심해야 하나"로 잇는 화면입니다.

| 각도 | 무엇을 보여주나 |
|---|---|
| **강도** | 이번 달 보도량이 그 유형 월평균의 몇 배인가 (예: 8월 폭염 4.8배) |
| **추세** | 최근 5년, 위험 보도 안에서 차지하는 **비중**이 늘었나 줄었나 |
| **장소** | 이번 달 그 위험 보도가 가장 몰린 공원 (예: 수난사고 → 속리산·지리산) |
| **사례** | 그 시기 보도가 가장 몰렸던 해의 실제 기사 1건 링크 |

**[연간]** 을 누르면 1~12월 각 달의 상위 위험 3종을 한눈에 볼 수 있고,
지도에서 **공원을 클릭하면** 사이드바에 그 공원만의 **시기별 안전 프로필**
(12개월 누적 차트 + 이번 달 상위 유형)이 함께 열립니다.

> ⚠️ 이 수치는 전부 **언론 보도량**입니다. 실제 사고 통계가 아니므로 화면에도
> 같은 단서를 붙여 두었습니다. 연도별 추세를 '건수'가 아니라 '비중'으로만 말하는
> 것도 같은 이유입니다 — 수집 깊이가 최근일수록 깊어 건수 비교는 왜곡됩니다.
...
**연간 순위는 어떻게 1년치가 있나요? 기능은 최근에 생겼는데.**
매일 아침 쌓는 기록은 2026년 8월 9일부터지만, 그 이전 12개월은 이 사이트가 모아둔 **뉴스 아카이브(자료받기의 5.7만 건)**를 같은 방식으로 묶어 사후 집계했습니다. 사후 집계분은 수집 경로가 달라 언론사 수가 실제보다 적게 잡힐 수 있어 파일에 `backfilled` 표시를 남겨 두었습니다.

**공원 면적·지정연대·세계 분포 통계는 어디 갔나요?**
2026년 8월에 통계 창을 **뉴스 통계 전용**으로 개편하면서 뺐습니다. 지금 [통계] 버튼을 누르면 기간별 **총보도량 · 하루평균 · 최다 보도 공원**과 **일별 보도량 추이 · 분야 구성 · 활발 언론사**가 나옵니다. 국립공원 자체 소개 통계는 공단 공식 자료 쪽이 더 정확해서, 이 사이트는 뉴스가 말해주는 것에 집중하기로 했습니다.
...
├─ search.html               # 보관 기사 검색 (제목·연도·공원·분야)
├─ data.html                 # 자료받기 — 뉴스 데이터 CSV 내려받기
├─ assets/
│  ├─ css/style.css          # 글래스모피즘 · 다크/라이트 지도 · 반응형
│  ├─ data/
│  │  ├─ parks-kr.geojson    # 국내 24곳 공식 경계 (0.83MB)
│  │  ├─ parks-global.json   # 해외 3,313곳 + 계층 (1.0MB, 지연 로드)
│  │  └─ safety-calendar.json # 안전 캘린더 사전 집계 (tools/build_safety_calendar.py)
│  └─ js/
...
├─ .github/workflows/        # ranking-retry.yml — 크론 불발 시 매시간 재시도
...
| 기간 | 읽는 파일 |
|---|---|
| 오늘 | 보관본 없이 즉석 수집 — CDN 30분 캐시 (`period=today`) |
| 주간 | 일간 보관본 7개 |
| 월간 | 일간 보관본 30개 |
...
> ⏱️ Vercel Hobby는 함수 최대 60초라 수집에 **42초 시간 예산**을 걸어 두었습니다(RSS 질의당 타임아웃 8초).
> 예산이 다하면 통째로 실패하는 대신 **그때까지 모은 것만이라도 저장**합니다 — 질의를 50개로 늘린 뒤
> 60초를 넘겨 크론이 전면 실패하던 문제(2026-08-16~17 공백)의 재발 방지책입니다.
> 📅 **보관본은 소급 생성되지 않습니다.** 구글 뉴스 RSS가 과거 날짜를 온전히 돌려주지 않기 때문입니다.
> 크론을 늦게 켤수록 온전한 월간·연간 순위도 그만큼 늦어집니다.

> 🔁 **Vercel 무료 크론은 발화 자체를 거르는 날이 있습니다** (2026-08-16·17, 08-20·21 실측 — 함수는
> 7초 만에 성공하는데 호출이 아예 없었습니다). 그래서 GitHub Actions
> (`.github/workflows/ranking-retry.yml`)가 **매시간** 완충 경로를 대신 두드립니다.
> 이미 보관된 날은 서버가 즉시 종료(커밋 0)하므로 부담이 없고, 크론이 걸러도 1시간 안에 메워집니다.
